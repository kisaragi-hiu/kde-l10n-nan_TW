...
- While Pe̍h-ōe-jī and Tâi-lô are often treated as separate systems, in reality Tâi-lô is a minor spelling reform on top of Pe̍h-ōe-jī. The way tones are marked #footnoteCite(<tailo-manual-tones>) remains almost identical; the convention of separating syllables within one compound with "-" instead of spaces is the same too. The only difference is that some consonants and vowels are spelled differently, such as `ch` in Pe̍h-ōe-jī being spelled `ts` in Tâi-lô, or `oe` in Pe̍h-ōe-jī being spelled `ue` in Tâi-lô, but the differences are one-to-one and unambiguous.
...
2. Translate: Translators then take existing PO files from the Subversion repository, translate them, then submit them. Some translators have direct commit access to the Subversion repository, so they can, after responsibly reviewing their submission, directly push translations into the repository; these translators include the language coordinators of their translation teams. Other translators may send their translations to language coordinators, in whichever way each translation team sees fit. For example, the Simplified Chinese team gathers translations via their own Crowdin project, and their language coordinator periodically synchronizes the translations on their Crowdin project with the `zh_CN` translations in the KDE Subversion repository.
...
I also had to make a compromise for the language name: although Taiwanese Taigi is now the officially recommended formal name of this language, this has not been recognized by international standards such as ISO 639-3 or by glibc locales. Thus, for the time being, until if and when ISO 639-3 CR 2021-044 #footnoteCite(<iso639-3-CR2021-044>) (establish Taigi as its own language with code `ftg`) is accepted, this language would have to be added with `nan_TW` as the language code and "Min Nan Chinese" as the English name.

== Translating for Taiwanese Taigi

As a Taiwanese Taigi language team does not already exist, I have to merge translation templates with my translations with my own automation. I also have to set up a translation "workspace" myself to most effectively utilize the capabilities of the Lokalize computer-aided translation system (also itself a KDE application) #footnoteCite(<lokalize>), which I rely on for my translations.

Lokalize provides translation memory features, glossary management, allows using other languages for reference, as well tracking translation progress across a whole tree of translation files, all for an offline folder of PO files. This translation process would've been far more tedious without Lokalize.
